...
data {
  ...
  int<lower=0> N_edges; // number of site-site connections (edges)
  int<lower=1, upper=n_sites> node1[N_edges];  // node1[i] adjacent to node2[i]
  int<lower=1, upper=n_sites> node2[N_edges];  // and node1[i] < node2[i]
...
parameters {
  ...
  // spatial effects
  real<lower=0> sigma;  // overall site-wise standard deviation
  real<lower=0, upper=1> rho; // proportion unstructured vs. spatially structured variance
  vector[n_sites] phi;  // spatially structured variance
  vector[n_sites] theta; // heterogenous site variance
  ...
  //vector[n_sites] psi_site; // heterogenous site specific intercept for occupancy
    ...
    for (j in 1:n_sites){    // loop across all sites
      for(k in 1:n_intervals){ // loop across all intervals  
          ...
          logit_psi[i,j,k] = // the inverse of the log odds of occurrence is equal to..
            ...
            convolved_re[sites[j]] * sigma // a spatial autocorrelation effect
            ; // end psi[i,j,k]
...
model {
  ...
  sum(phi) ~ normal(0, 0.001 * n_sites);  // equivalent to mean(phi) ~ normal(0,0.001)
