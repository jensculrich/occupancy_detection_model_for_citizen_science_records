...
data {
  ...
  int<lower=1, upper=n_level_three> level_three_lookup[n_sites]; // level-3 cluster look up vector for level-3 cluster
  int<lower=1, upper=n_level_four> level_four_lookup[n_level_three]; // level-4 cluster look up vector for level-4 cluster
...
transformed parameters {
  ...
  vector[n_level_four] psi0_level_four;

  vector[n_sites] p0_cs_site;
  vector[n_level_three] p0_cs_level_three;
  vector[n_level_four] p0_cs_level_four;
  
  vector[n_sites] p0_rc_site;
  vector[n_level_three] p0_rc_level_three;
  vector[n_level_four] p0_rc_level_four;

  // Level-4
  psi0_level_four = psi_level_four * sigma_psi_level_four;
  ...
  psi0_level_three = psi0_level_four[level_four_lookup] + psi_level_three * sigma_psi_level_three;

  // compute varying intercept at the site level
  // Level-2 (n_sites level-2 random intercepts, nested in ecoregion3)
  psi0_site = psi0_level_three[level_three_lookup] + psi_site * sigma_psi_site;
  
  // Level-4
  p0_rc_level_four = p_rc_level_four * sigma_p_rc_level_four;
  
  // compute the varying community science detection intercept at the ecoregion3 level
  // Level-3 (n_level_three level-3 random intercepts)
  p0_rc_level_three = p0_rc_level_four[level_four_lookup] + p_rc_level_three * sigma_p_rc_level_three;

  // compute varying intercept at the site level
  // Level-2 (n_sites level-2 random intercepts, nested in ecoregion3)
  p0_rc_site = p0_rc_level_three[level_three_lookup] + p_rc_site * sigma_p_rc_site;
  
  // Level-4
  p0_cs_level_four = p_cs_level_four * sigma_p_cs_level_four;
  
  // compute the varying community science detection intercept at the ecoregion3 level
  // Level-3 (n_level_three level-3 random intercepts)
  p0_cs_level_three = p_cs_level_four[level_four_lookup] + p_cs_level_three * sigma_p_cs_level_three;

  // compute varying intercept at the site level
  // Level-2 (n_sites level-2 random intercepts, nested in ecoregion3)
  p0_cs_site = p0_cs_level_three[level_three_lookup] + p_cs_site * sigma_p_cs_site;
