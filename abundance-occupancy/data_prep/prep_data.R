...
prep_data <- function(era_start, era_end, n_intervals, n_visits, 
  ...
  source("./abundance-occupancy/data_prep/get_spatial_data.R")
  ...
  for(i in 1:n_intervals){
    for(j in 1:n_visits){
      ...
      temp <- df_citsci %>%
        ...
        # group by SPECIES
        group_by(species, grid_id, occ_interval, visit) %>%
        count() %>%
        # spread sites by species, and fill with 0 if species never captured this interval*visit
        spread(grid_id, n, fill = 0) %>%

        # if more columns are added these indices below^ might need to change
  ...
  ## --------------------------------------------------
  # Get species ranges
  source("./abundance-occupancy/data_prep/get_species_ranges.R")
