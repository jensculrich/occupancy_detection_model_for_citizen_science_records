...
parameters {
  ...
  real psi_interval; // vector of species specific slope estimates
    ...
    for (j in 1:n_sites){    // loop across all sites
      for(k in 1:n_intervals){ // loop across all intervals  
          ...
          psi[i,j,k] = inv_logit( // the inverse of the log odds of occurrence is equal to..
            ...
            psi_interval*intervals[k] + // a species specific temporal effect
...
model {
  ...
  // distribution (variance defined by sigma), centered at 0. 
  sigma_psi_site ~ cauchy(0, 1);
  
  psi_interval ~ cauchy(0, 1); // tightened prior
