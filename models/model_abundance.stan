  ...
  for (i in 1:n_species){   // loop across all species
    for (j in 1:n_sites){    // loop across all sites
      for(k in 1:n_intervals){ // loop across all intervals  
          ...
          lambda[i,j,k] = exp( // the inverse of the log odds of occurrence is equal to..
  ...
  for(i in 1:n_species) { // loop across all species
    for(j in 1:n_sites) { // loop across all sites
      for(k in 1:n_intervals){ // loop across all intervals
          ...
          // need to figure out how to impose the NA structure
          
          vector[K - max_y[i,j,k] + 1] lp; // lp vector of length of possible abundances 
            // (from max observed to K) 
          
          for (abundance in 1:(K - max_y[i,j,k] + 1)) // for each possible abundance:
          // lp of abundance given ecological model and observational model
          lp[abundance] = neg_binomial_2_lpmf(max_y[i,j,k] + abundance - 1 | exp(lambda[i,j,k]), phi)
            + binomial_logit_lpmf(V_citsci[i,j,k] | max_y[i,j,k] + abundance - 1, p_citsci[i,j,k]); // vectorized over n visits..
